...
def low_pass_filter(log_new_frame, lp_log_frame, inten01, delta_time, cutoff_hz=0):
    ...
    if inten01 is not None:
        ...
        if max_eps > 0.3:
            ...
            if low_pass_filter.iir_warning_count < IIR_MAX_WARNINGS:
                ...
                if low_pass_filter.iir_warning_count == IIR_MAX_WARNINGS:
                    logger.warning(
                        f"Supressing further warnings about inaccurate IIR lowpass filtering; check timestamp resolution and DVS photoreceptor cutoff frequency"
                    )
...
def compute_photoreceptor_noise_voltage(
    ...
    rate_per_bw = (
        shot_noise_rate_hz / f3db
    ) / 2  # simulation data are on ON event rates, divide by 2 here to end up with correct total rate
    if rate_per_bw > 0.5:
        logger.warning(
...
def generate_shot_noise(
    ...
    shot_on_cord = torch.gt(rand01, one_minus_shot_ON_prob_this_sample.unsqueeze(0))
    shot_off_cord = torch.lt(rand01, shot_OFF_prob_this_sample.unsqueeze(0))
